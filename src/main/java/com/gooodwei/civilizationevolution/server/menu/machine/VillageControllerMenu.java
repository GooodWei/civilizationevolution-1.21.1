 ...
 * </ul>
 *
 * <p>与 {@link PrimitiveControllerMenu} 布局完全一致，仅方块实体类型不同。
 */
public class VillageControllerMenu extends MachineMenu {
    /** 底层的方块实体容器引用 */
     ...
     * @param blockEntity     村庄控制器方块实体
     * @param containerData   同步数据
     */
    public VillageControllerMenu(int containerId, Inventory playerInventory, VillageControllerBlockEntity blockEntity, ContainerData containerData) {
        super(MenuRegistry.VILLAGE_CONTROLLER_MENU.get(), containerId);
     ...
     * @return 重建的 VillageControllerMenu 实例（使用占位 {@link SimpleContainerData}）
     */
    public static VillageControllerMenu fromNetwork(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf buf) {
        BlockPos blockPos = buf.readBlockPos();
        VillageControllerBlockEntity blockEntity = (VillageControllerBlockEntity) playerInventory.player.level().getBlockEntity(blockPos);
        return new VillageControllerMenu(containerId, playerInventory, blockEntity, new SimpleContainerData(3));
    }
    ...
    public BlockPos getBlockPos() {
        return ((VillageControllerBlockEntity) this.container).getBlockPos();
    }
