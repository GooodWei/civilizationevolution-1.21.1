 ...
 * TODO：当前槽位坐标为牧场占位 GUI 的临时值，农场专用 GUI 设计完成后需调整。</p>
 */
public class VillageFarmMenu extends MachineMenu {
     ...
     * @param blockEntity     村庄农场方块实体
     * @param data            同步数据（3 个字段）
     */
    public VillageFarmMenu(int containerId, Inventory playerInventory,
                              VillageFarmBlockEntity blockEntity, ContainerData data) {
        super(MenuRegistry.VILLAGE_FARM_MENU.get(), containerId);
     ...
     * @return 重建的 VillageFarmMenu 实例（使用占位 {@link SimpleContainerData}）
     */
    public static VillageFarmMenu fromNetwork(int containerId, Inventory playerInventory,
                                                 RegistryFriendlyByteBuf buf) {
        VillageFarmBlockEntity be = (VillageFarmBlockEntity)
                playerInventory.player.level().getBlockEntity(buf.readBlockPos());
        return new VillageFarmMenu(containerId, playerInventory, be,
    ...
    public BlockPos getBlockPos() {
        return ((VillageFarmBlockEntity) this.container).getBlockPos();
    }
    ...
    public void removed(Player player) {
        ...
        if (!player.level().isClientSide && container instanceof VillageFarmBlockEntity be) {
