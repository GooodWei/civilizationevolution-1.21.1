 ...
 * 流体输入接口的抽象基类。
 *
 * <p>每个流体输入接口含内部流体储罐，接受外部流体输入。
 * 多方块控制器可通过此接口消耗流体（水、熔岩等）。
 * 无 GUI，纯通过 {@code IFluidHandler} 与物流管道交互。
 * 子类只需覆写 {@link #codec()}、{@link #newBlockEntity} 和 {@link #getPartTier()}。
 */
public abstract class AbstractFluidInputHatchBlock extends AbstractHatchBlock {

    protected AbstractFluidInputHatchBlock(Properties properties) {
        super(properties);
    }
